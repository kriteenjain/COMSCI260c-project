"""Compress LLaMA (Wanda or AWQ) and evaluate on all five tasks.

Drop-in analogue of run_qwen_compression.py with LLaMA 3.2-3B defaults.
The eval pipeline and JSON schema are identical so result files from both
scripts can be compared directly.

Examples (Colab workflow — GPU runtime required):

    # Wanda unstructured 50%, all tasks, 100 examples each
    python run_llama_compression.py --method wanda --task both --limit 100

    # Wanda 2:4 structured
    python run_llama_compression.py \\
        --method wanda --sparsity-type 2:4 \\
        --task both --limit 100 \\
        --save-compressed ./compressed/llama-wanda-2-4

    # AWQ INT4 (CUDA-only)
    python run_llama_compression.py --method awq --task both --limit 100 \\
        --save-compressed ./compressed/llama-awq-w4g128

    # Re-evaluate a cached compressed model
    python run_llama_compression.py --method awq --task both --limit -1 \\
        --load-compressed ./compressed/llama-awq-w4g128
"""
...
def _compress_awq(args) -> tuple[LM, dict]:
    ...
    if args.load_compressed:
        ...
        quant_path = args.save_compressed or f"./compressed/llama-awq-w{args.w_bit}g{args.q_group_size}"
        quantize_with_awq(
            args.model,
            quant_path,
            w_bit=args.w_bit,
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Compress LLaMA-3.2-3B with Wanda or AWQ and evaluate on all tasks.",
    )
    ...
    # Eval
    ap.add_argument("--task", choices=["gsm8k", "humaneval", "musique", "nq_open", "squad", "both"], default="both")
    ap.add_argument("--limit", type=int, default=100,
                    help="Number of examples per task. -1 for full split.")
    ap.add_argument("--batch-size", type=int, default=1,
                    help="LLaMA 3B fits T4 at batch=1; bump to 2 on A100/L4.")
